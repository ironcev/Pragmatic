...
namespace Pragmatic.Tests.Unit.Interaction
{
    ...
    public class NamespacedKeyResourceResponseMapperTests
    {
        ...
        public void Map_KeyIsNotOverloaded_KeepsOriginalMessage()
        {
            ...
            Assert.That(originalResponse.Information.First().Key, Is.EqualTo("TestEntityResource.SecondKey"));
            Assert.That(originalResponse.Information.First().Message, Is.EqualTo(DomainResource.TestEntityResource.SecondKey));

            // The other layer, e.g. the client code, does not have the message overridden in its own resource.
            var mappedResponse = _mapper.Map(originalResponse);
            Assert.That(mappedResponse.Information.First().Key, Is.EqualTo("TestEntityResource.SecondKey")); // The key stays the same.
            Assert.That(mappedResponse.Information.First().Message, Is.EqualTo(DomainResource.TestEntityResource.SecondKey)); // The message is translated.
        }
